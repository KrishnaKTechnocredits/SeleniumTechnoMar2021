...
import technocredits.utility.PropOperation;

public class TableEx4PropModular {
	...
	static void verifyDeptData(HashMap<String,Integer> mapOfDepts) throws IOException {
		PropOperation propOperation = new PropOperation(".//resources/deptInfo.properties");
		Set<String> keys = mapOfDepts.keySet();
		Iterator<String> itr = keys.iterator();
		while(itr.hasNext()) {
			String currentKey = itr.next();
			int deptCountExpected = Integer.parseInt(propOperation.getValue(currentKey));
